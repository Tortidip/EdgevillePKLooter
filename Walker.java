...
public class Walker {
     ...
     */
    private scripts.Walker.Type getType(String s) {
        if (s.matches("(?i)((.*)?door)")) {
            return scripts.Walker.Type.DOOR;
        } else if (s.matches("(?i)(gate(.*)?)")) {
            return scripts.Walker.Type.GATE;
        } else if (s.equalsIgnoreCase("ladder")) {
            return scripts.Walker.Type.LADDER;
        }
        return scripts.Walker.Type.TEAPOT;
    }
